...
echo -e "\e[36m
------------------------------------------
        Brendan's Luxury Arch Installer - Lite  
------------------------------------------
\e[0m"
...
parted -s /dev/$ddrive mkpart primary ext4 550MiB 100%


mkfs.fat -F32 /dev/"$dinstall"1
mkswap /dev/"$dinstall"2
swapon /dev/"$dinstall"2
mkfs.ext4 -l x /dev/"$dinstall"3
...
usermod -aG wheel,audio,video,optical,storage,sudo $newuname
# set timezone
